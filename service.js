...
const clientSecret = process.env.SPOTIFY_CLIENT_SECRET
  ...
  async function getPlaylists(accessToken) {
    try {
      const response = await axios.get('https://api.spotify.com/v1/users/3157dolwsbzippqyy6yrbbjislx4/playlists', {
        headers: {
...
async function getPlaylistDetails(accessToken, playlistId) {
    ...
    try {
      ...
      response.data.items.map(item => {
        ...
        const songObject = { artist: artists, artist_id: artistId, track: track.name, track_id, duration: durationMinutesSeconds, albumName: name, albumID: id }
