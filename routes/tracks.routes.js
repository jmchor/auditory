        ...
        if (existingTrack.rows.length > 0) {
            ...
            if (
                ...
                if (updateResult.rows.length > 0) {
                    ...
                    failedEntries.push({ track_id, reason: 'No matching track found' });
                }
            } else {
                // No additional info to update
                console.log(`Track '${trackName}' already exists with complete info. Skipping update.`);
            }
        } else {
...
res.json({ success: true, message: 'Tracks fetched and processed successfully. Here are the exceptions: ', failedEntries });
